...
Everything that can be automated is. What is left is the eight things below, which
...
A page that nobody turns off keeps running. The workflow still has its
schedule, the Apps Script still has its triggers, and nothing in either knows
the night has been and gone.

This is not hypothetical. One finished page kept republishing seven Datawrapper
charts on every scheduled run for a week afterwards and reached version 248 —
roughly 1,700 API calls spent on figures that had stopped changing. The
Datawrapper quota is shared across the organisation, so that is taken from
whichever election is next.

Once the result is final:

- [ ] **Actions → Update election data → Disable workflow.** The page keeps
      serving the committed `data/*.json`; nothing disappears.
- [ ] **Apps Script → Triggers:** delete the `handleSheetEdit` trigger and run
      `removeHeartbeat()`. A Sheet that is still being tidied up should not be
      dispatching.
- [ ] Revoke the tokens that were only needed for this page: `GITHUB_PAT` in
      Script Properties, and the Datawrapper token if it was made for this
      election.
- [ ] Set `automation.datawrapper.republishOnUpdate` to `false` if it was on.

Two of these are now guarded in code rather than left to memory:
`republish-charts.js` stops republishing `automation.republishUntilDays` days
after `election.date.iso` (7 by default), and `republishOnUpdate` ships as
`false`. Neither guard stops the workflow itself, so the first item on the list
